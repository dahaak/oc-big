...
//			$this->copy_file_to_openclinica_dir($file->name);
    ...
    /* Modifications to integrate OpenClinica with File Upload Plugin - prav 290313 */
    ...
    protected function get_patient_id_param() {
        parse_str(parse_url($this->get_referer_url(), PHP_URL_QUERY), $parsed_referer_query);
        return isset($parsed_referer_query['id']) ? $parsed_referer_query['id'] : null;
	}
    ...
    /* Obtains the path to the openclinica directory  and concatenates the file name to it. - prav 130413 */
    protected function get_openclinica_upload_path($file_name = null, $version = null) {
        $file_name = $file_name ? $file_name : '';
	$version_path = empty($version) ? '' : $version.'/';
	return $this->options['openclinica_upload_dir'].$this->get_user_path()
	       .$version_path.$file_name;
    }

    /* Checks if the file is a zip file by trying to open it, takes in file name and concatenates path - prav 130413 */
    protected function is_zip_valid($zip_file_name) {
        $zip_chk = zip_open($this->get_upload_path($zip_file_name));
        if (is_resource($zip_chk)) {
            zip_close($zip_chk);
            return true;
        } else {
            return false;
    ...
    /* Validates zip file, extracts contents, appends openclinica parameters to file name and deletes original zip file - prav 130413 */
    /* If the zipped file contains sub directories, this code does not handle it and will break !! */
    protected function check_if_zip_and_unzip($zip_file_name) {
        $zip = new ZipArchive;
    ...
    unlink($this->get_upload_path($zip_file_name));
    }
